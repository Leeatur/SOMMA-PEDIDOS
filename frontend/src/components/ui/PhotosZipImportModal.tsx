      ...
      {result ? (
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-emerald-50 rounded-xl p-3 text-center">
              ...
              <p className="text-[11px] text-emerald-700">Vinculadas</p>
            </div>
            <div className={`rounded-xl p-3 text-center ${result.skipped > 0 ? 'bg-surface-container-low' : 'bg-surface-container-low'}`}>
              ...
              <p className="text-[11px] text-outline">Ignoradas*</p>
            </div>
            <div className={`rounded-xl p-3 text-center ${result.notFound > 0 ? 'bg-amber-50' : 'bg-surface-container-low'}`}>
              ...
              <p className={`text-[11px] ${result.notFound > 0 ? 'text-amber-700' : 'text-outline'}`}>
                Ref. não encontrada
              </p>
            </div>
          </div>
          ...
          {result.errors.length > 0 && (
            <div className="bg-red-50 rounded-lg p-3">
              ...
              <p className="text-[11px] text-red-600 font-mono">{result.errors.join(', ')}</p>
            </div>
          )}
        </div>
      ) : (
        ...
        <div className="space-y-1">
            ...
            >
              {file ? (
                <div className="flex items-center justify-center gap-2 text-primary">
                  ...
                  <span className="text-[11px] text-outline/70">({(file.size / 1024 / 1024).toFixed(0)} MB)</span>
                </div>
              ) : (
                <div className="text-outline/70">
                  <Upload className="h-8 w-8 mx-auto mb-2" />
                  <p className="text-[11px]">Clique para selecionar o arquivo .zip</p>
                  <p className="text-[11px] mt-0.5 text-outline/50">Qualquer tamanho — processado localmente</p>
                </div>
              )}
            </div>
            <input
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-[11px] text-on-surface-variant">Sobreescrever fotos já existentes</span>
          </label>
          ...
          {/* Erro */}
          {error && (
            <div className="flex items-start gap-2 bg-red-50 rounded-xl p-3 text-[11px] text-red-600">
