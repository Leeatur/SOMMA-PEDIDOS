    ...
    <Modal open={open} onClose={handleClose} title="Importar Clientes via Excel" size="lg">
        ...
        {(['upload', 'mapping', 'result'] as Step[]).map((s, i) => {
          ...
          return (
            <div key={s} className="flex items-center gap-2 flex-1">
              <div className={`flex items-center gap-1.5 text-[12px] font-semibold ${active ? 'text-primary' : done ? 'text-emerald-600' : 'text-outline/70'}`}>
                <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[12px] font-bold ${active ? 'bg-primary/10 text-primary ring-2 ring-blue-300' : done ? 'bg-emerald-100 text-emerald-700' : 'bg-surface-container text-outline/70'}`}>
      ...
      {step === 'upload' && (
        <div className="space-y-1">
          ...
          >
            {previewMut.isPending ? (
              <div className="flex flex-col items-center gap-3 text-primary">
                ...
                <p className="text-[12px] font-medium">Lendo arquivo...</p>
              </div>
            ) : (
              <div className="flex flex-col items-center gap-3 text-outline">
                ...
                <div>
                  ...
                  <p className="text-[12px]">ou clique para selecionar</p>
                </div>
                <p className="text-[12px] text-outline/70">Suporta .xlsx e .xls</p>
              </div>
            )}
          </div>
          ...
          {previewMut.isError && (
            <p className="text-[12px] text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-1 flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 flex-shrink-0" />
              {(previewMut.error as any)?.response?.data?.error || 'Erro ao ler o arquivo'}
            </p>
          )}

          <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2 text-[12px] text-amber-800 space-y-1">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-[12px] text-outline hover:text-on-surface-variant flex items-center gap-1"
            >
          ...
          <div className="space-y-1">
            <p className="text-[12px] font-semibold text-outline uppercase tracking-wide">
              ...
              {FIELDS.map(f => (
                <div key={f.key} className="flex items-center gap-3">
                  ...
                  <select
                    ...
                    className={`flex-1 text-[12px] border rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-400 ${
        ...
        <div className="space-y-1.5">
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
              ...
              <p className="text-[12px] text-emerald-600 font-medium mt-0.5">Importados</p>
            </div>
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
              <p className="text-2xl font-bold text-amber-700">{result.skipped}</p>
              <p className="text-[12px] text-amber-600 font-medium mt-0.5">Já existiam</p>
            </div>
            <div className={`border rounded-xl p-4 text-center ${result.errors.length ? 'bg-red-50 border-red-200' : 'bg-surface-container-low border-outline-variant'}`}>
              ...
              <p className={`text-[12px] font-medium mt-0.5 ${result.errors.length ? 'text-red-600' : 'text-outline'}`}>Erros</p>
            </div>
          </div>
          ...
          {result.skipped > 0 && (
            <p className="text-[12px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1">
          ...
          {result.errors.length > 0 && (
            <div className="border border-red-200 rounded-lg overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-1 bg-red-50 text-[12px] font-medium text-red-700"
              >
              ...
              {showErrors && (
                <ul className="divide-y divide-red-100 max-h-40 overflow-y-auto">
                  {result.errors.map((e, i) => (
                    <li key={i} className="px-3 py-1.5 text-[12px] text-red-600">{e}</li>
                  ))}
                </ul>
              )}
            </div>
