      ...
      <div className="bg-white border-b border-outline-variant px-5 py-2.5 lg:px-8">
        <div className="max-w-3xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-[13px] text-outline mt-0.5">{list.length} usuários</p>
          </div>
                ...
                <Card key={u.id} padding="md">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[13px] text-outline">{u.email}</p>
                        ...
                        <div className="flex items-center gap-1 mt-1 flex-wrap">
                          ...
                          {factoryNames.length > 0 ? (
                            factoryNames.map(name => (
                              <span key={name} className="text-[12px] bg-primary/10 text-primary px-1.5 py-0.5 rounded font-medium">
                                {name}
                              </span>
                            ))
                          ) : (
                            <span className="text-[12px] text-outline/60 italic">todas as fábricas</span>
                          )}
                        </div>
                      )}
                      <p className="text-[13px] text-outline/70">Criado em {formatDate(u.created_at)}</p>
                    </div>
                    <div className="flex items-center gap-1 flex-shrink-0">
                      {confirmDeleteId === u.id ? (
                        <div className="flex items-center gap-1">
                          <span className="text-[13px] text-red-600 font-medium">Confirmar?</span>
                          <button
                            onClick={() => deleteMut.mutate(u.id)}
                            disabled={deleteMut.isPending}
                            className="px-2 py-1 text-[13px] bg-red-500 text-white rounded-lg hover:bg-red-600 disabled:opacity-50 transition-colors"
                          >
                            Sim
                          </button>
                          <button
                            onClick={() => setConfirmDeleteId(null)}
                            className="px-2 py-1 text-[13px] border border-outline-variant text-outline rounded-lg hover:bg-surface-container transition-colors"
                          >
                            Não
                          </button>
                        </div>
        ...
        <div className="space-y-1">
          ...
          {form.role === 'representante' && factories.length > 0 && (
            <div>
              ...
              <p className="text-[13px] text-outline mb-2">
