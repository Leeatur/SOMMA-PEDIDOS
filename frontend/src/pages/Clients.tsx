...
export function Clients() {
      ...
      case 'name':
        return (
          <td key={id} className="pl-3 pr-2 py-0 max-w-[220px]">
            <p className="text-[12px] font-semibold text-on-surface truncate leading-none">{c.name}</p>
          </td>
        )
      case 'trade_name':
        return (
          <td key={id} className="px-2 py-0 max-w-[180px]">
            <span className="text-[12px] text-outline truncate block">
      ...
      case 'phone':
        return (
          <td key={id} className="px-2 py-0 whitespace-nowrap">
            <span className="text-[12px] text-on-surface-variant">{c.phone || '—'}</span>
          </td>
        )
      case 'whatsapp':
        return (
          <td key={id} className="px-2 py-0 whitespace-nowrap">
            <span className="text-[12px] text-on-surface-variant">{c.whatsapp || '—'}</span>
          </td>
        )
      case 'email':
        return (
          <td key={id} className="px-2 py-0 max-w-[180px]">
            <span className="text-[12px] text-outline truncate block">{c.email || '—'}</span>
          </td>
        )
      case 'cnpj':
        return (
          <td key={id} className="px-2 py-0 whitespace-nowrap">
            <span className="text-[12px] text-outline/70">{c.cnpj || '—'}</span>
          </td>
        )
      case 'rep':
        return (
          <td key={id} className="px-2 py-0 max-w-[120px]">
            <span className="text-[12px] text-primary font-medium truncate block">{c.rep_name || '—'}</span>
          </td>
        )
      case '_edit':
        return (
  ...
  if (isError) return (
    <div className="flex flex-col items-center justify-center h-full py-24 text-center px-8">
      ...
      <p className="text-[12px] text-outline/70">Verifique sua conexão e tente novamente.</p>
      <button onClick={() => window.location.reload()} className="mt-4 text-[12px] text-primary underline">
        Recarregar
      </button>
    </div>
  )
        ...
        <div className="px-4 pt-3 pb-2 bg-white border-b border-outline-variant/60 shadow-sm">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-11 pl-10 pr-10 bg-surface-container-low border border-outline-variant/60 rounded-xl text-[12px] focus:ring-2 focus:ring-primary focus:border-primary outline-none"
            />
            {search && (
                  ...
                  <div key={c.id} className="bg-white rounded-xl border border-outline-variant/40 shadow-sm overflow-hidden active:bg-surface-container-low transition-colors">
                    <div className="flex items-start gap-2 p-2 cursor-pointer" onClick={() => openEdit(c)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-semibold text-on-surface text-[12px] leading-tight truncate">{c.name}</p>
                        {c.trade_name && c.trade_name !== c.name && (
                          <p className="text-[12px] text-outline truncate mt-0.5">{c.trade_name}</p>
                        )}
                        {(c.city || c.state) && (
                        ...
                        {c.cnpj && (
                          <p className="text-[12px] text-outline mt-0.5 font-mono">{c.cnpj}</p>
                        )}
                      </div>
                      {/* Edit / Delete */}
                    ...
                    {(c.phone || c.whatsapp || c.email) && (
                      <div className="flex border-t border-outline-variant/30 divide-x divide-outline-variant/30">
                        {c.phone && (
                          <a href={`tel:${c.phone}`}
                             className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[12px] font-semibold text-on-surface-variant hover:bg-surface-container active:bg-surface-container transition-colors"
                        ...
                        {c.whatsapp && (
                          <a href={`https://wa.me/55${c.whatsapp.replace(/\D/g, '')}`}
                             ...
                             className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[12px] font-semibold text-emerald-700 hover:bg-emerald-50 active:bg-emerald-100 transition-colors"
                             onClick={e => e.stopPropagation()}>
                            💬 WhatsApp
                          </a>
                        )}
      ...
      <div className="px-4 pt-4 pb-2 lg:px-8 border-b border-outline-variant bg-white">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setShowNewCnpj(true)}
              className="flex items-center gap-1.5 text-[12px] font-semibold text-white bg-primary hover:bg-primary/90 rounded-lg px-3 py-1 transition-colors"
            >
          ...
          {!search && (
            <div className="flex gap-3 mt-4">
              <button onClick={() => setShowImport(true)}
                className="flex items-center gap-2 px-4 py-1 bg-surface-container text-on-surface-variant text-[12px] font-semibold rounded-lg hover:bg-surface-container-high transition-colors">
                <Upload className="h-4 w-4" /> Importar Excel
              </button>
              <button onClick={() => setShowNewCnpj(true)}
                className="flex items-center gap-2 px-4 py-1 bg-primary text-white text-[12px] font-semibold rounded-lg hover:bg-primary/90 transition-colors">
                ...
                {visibleCols.map(col => {
                  ...
                  return (
                    <th
                      ...
                      className={`px-2 py-1.5 text-[12px] font-semibold text-outline first:pl-3 last:pr-3 ${col.id === '_edit' ? 'w-10' : ''} ${sortable ? 'cursor-pointer select-none hover:text-on-surface' : ''}`}
                    >
